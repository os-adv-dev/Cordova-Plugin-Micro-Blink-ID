...
function getGoogleServiceTargetDir(context) {
    ...
    switch (platform) {
        ...
        case "ios":
            return path.join(context.opts.plugin.dir, "/src/ios/libs");
            // return platformPath;
        default:
            return undefined;
    }
...
module.exports = function(context) {
    return new Promise(function(resolve, reject) {
        ...
        var configPath = path.join(context.opts.plugin.dir, "/src/ios/libs");
        console.log(configPath);

        var prefZipFilename = "MicroBlinkFramework";
        var zipFile = getZipFile(configPath, prefZipFilename);
        // if zip file is present, lets unzip it!
        if (!zipFile) {
            return reject(
                "Failed to install MicroblinkID plugin. Reason: MicroblinkID framework zip file not found."
            );
        }
        var unzipedResourcesDir = unzip(zipFile, configPath, prefZipFilename);
        // var platform = context.opts.plugin.platform;
        // var targetDir = getTargetDir(context);
        // var copyWithSuccess = copyGoogleServiceFile(
        //     unzipedResourcesDir,
        //     path.join(context.opts.plugin.dir, "/src/ios/libs"),
        //     platform
        // );

        // if (!copyWithSuccess) {
        //     return reject(
        //         "Failed to install DynamicLinks plugin. Reason: Unable to copy google services file to project."
        //     );
        // }
        return resolve();
    });
